...
ImageManager.loadBitmap = function(folder, filename, hue, smooth) {
    if (filename) {
        var path = folder +filename.toLowerCase() + '.png';
...
ImageManager.loadNormalBitmap = function(path, hue) {
    ...
    if (!bitmap) {
        bitmap = Bitmap.load(path.toLowerCase());
...
(function() {
    PluginManager = class extends PluginManager {
        ...
        static loadScript(name) {
            ...
            let buff = window["data/js/plugins"][loadname];
            var url = this._path + name;
            var script = document.createElement('script');
            script.type = 'text/javascript';
            script.innerHTML = `/*${loadname}*/\n${buff}`;
            script._url = url;
            document.body.appendChild(script);
